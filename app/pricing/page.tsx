...
export const metadata: Metadata = {
  ...
  description:
    "Decouvrez nos plans tarifaires simples et transparents pour la gestion de votre elevage porcin.",
};
            ...
            >
              ...
              <div className="mb-4">
                ...
                {plan.price !== "Sur devis" && (
                  <span className="text-sm"> FCFA/mois</span>
                )}
              </div>
              <p
                className={`mb-6 ${(plan as any).popular ? "text-primary-foreground/80" : "text-muted-foreground"}`}
              >
                {plan.description}
              </p>
              <ul className="space-y-3 mb-8">
                {plan.features.map((feature, j) => (
                  <li key={j} className="flex items-start gap-3">
                    <Check
                      className={`h-5 w-5 shrink-0 ${(plan as any).popular ? "" : "text-primary"}`}
                    />
